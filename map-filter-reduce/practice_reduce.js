...
console.log("rows")
console.log(rows)
let rowss = rows.reduce((result, item) => ({
	  ...result,
...
let data = [{ A: 4, B: 2 }, { A: 2, B: 1 }, { A: 3, B: 1 }, { A: 2, B: 1, C: 1 }]

    result4 = data.reduce((r, o) => (Object.entries(o).forEach(([k, v]) => r[k] = (r[k] || 0) + v), r), {}); 
...
let x = array1.reduce((accu,value)=>{return accu+value})
 x = array1.reduce(function(accu,value){return accu+value})
console.log(x)
...
let adf = playersl.reduce((accu,item,index)=>{
	item.name==="entrada"?accu[item.id]=item.name:accu[item.id]=item.name 
	return accu
},{},{})
...
let akafjalf= arraydsfds.reduce((accumulator, current) => ({
    ...accumulator,
